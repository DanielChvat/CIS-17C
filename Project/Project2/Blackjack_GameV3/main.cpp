...
void print(char);
int cHash(char);
bool hshSrch(char, LinkedList<char> **, const int);
...
int main(){
    ...
    char search = '\0';
    const int NCHARS = 13;
    LinkedList<char> **hMap = new LinkedList<char> *[NCHARS];
    //Initialize Variables
    //Create all 3 file classes
    for(int i=0; i<NFILES; i++)file[i] = new File();
    for(int i=0; i<NCHARS; i++)hMap[i] = new LinkedList<char>;
    ...
    if(game.getInfo()=='Y' || game.getInfo()=='y'){
        ...
        cout<<endl;
        for(int i=0; i<deck.getNCrd(); i++)hMap[cHash(stats->card[i])%NCHARS]->addLst(stats->card[i]);
        cout<<"Would You Like To Search the Deck To See If That Card is In the Deck? (Y/N)"<<endl;
        cin>>info;
        while(info == 'Y' || info == 'y'){
            cout<<"Please Input The Card You Would Like To Search For"<<endl;
            char search = '\0';
            cin>>search;
            if(hshSrch(search, hMap, NCHARS))cout << endl << search << " Is In The Deck" << endl;
            else cout << endl << search << " Is Not In The Deck" << endl;
            cout<<"Would You Like to Search For Another Card? (Y/N)"<<endl;
            cin>>info;
        }
        cout<<"Sorted Card: ";
        Tree tree;
    ...
    //De-Allocate Memory
    destroy(stats);
    for(int i=0; i<NCHARS; i++)delete hMap[i];
    delete []hMap;
    //Exit Stage Right
    return 0;
}
...
int cHash(char n){
    int a = 0x352F8C; //Arbitrary Value I Randomly Generated 
    return (a + static_cast<int>(n));
}

bool hshSrch(char find, LinkedList<char > **hMap, const int SIZE){
    int index = cHash(find)% SIZE;
    if(hMap[index]->findLst(find) != -1) return true;
    else return false;
}